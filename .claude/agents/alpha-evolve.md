...
- Algorithmic implementations (sorting, search, optimization, indicators)
- Core strategy logic where correctness is critical
- Performance-critical code paths
- User explicitly requests "explore alternatives"

## NautilusTrader Context

This agent is specialized for NautilusTrader strategy development:
...
- Algorithm complexity (O(n^2) vs O(n log n) vs O(n))
- Design patterns (functional vs OOP vs hybrid)
- Library usage (pure implementation vs NautilusTrader native)
...
## NautilusTrader-Specific Approaches
...
class MyStrategy(Strategy):
    def on_bar(self, bar: Bar) -> None:
        new_state = self.state_machine.process(bar)
...
### Indicator Implementation Variants

```python
# Approach A: Native Rust indicator (preferred)
from nautilus_trader.indicators import ExponentialMovingAverage
self.ema = ExponentialMovingAverage(period=20)

# Approach B: Custom Python indicator
class CustomIndicator(Indicator):
    def __init__(self, period: int):
        self.period = period
        self._values = deque(maxlen=period)

# Approach C: Vectorized calculation (for backtesting)
def calculate_indicator(bars: pd.DataFrame) -> pd.Series:
    return bars['close'].ewm(span=20).mean()
```
