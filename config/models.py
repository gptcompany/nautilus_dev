...
class ConfigEnvironment(BaseModel):
    """Core environment configuration (T005)."""
...
class RedisConfig(BaseModel):
    """Redis connection configuration (T006)."""
...
class StreamingSettings(BaseModel):
    """Streaming/catalog configuration (T008)."""
...
class ExchangeCredentials(BaseModel):
    """Base exchange credentials (T009)."""
    ...
    def validate_not_placeholder(cls, v: str) -> str:
        """Reject common placeholder patterns in API credentials."""
        v_lower = v.lower()
        ...
        for pattern in placeholder_patterns:
            if pattern in v_lower:
                raise ValueError(f"Placeholder credentials not allowed (detected: '{pattern}')")
        return v
...
class TradingNodeSettings(BaseModel):
    ...
    # Rate limits (format: "count/HH:MM:SS")
    max_order_submit_rate: str = Field(default="100/00:00:01")
    max_order_modify_rate: str = Field(default="100/00:00:01")

    @field_validator("max_order_submit_rate", "max_order_modify_rate")
    @classmethod
    def validate_rate_limit_format(cls, v: str) -> str:
        """Validate rate limit format matches 'count/HH:MM:SS' with count >= 1."""
        import re

        # Rate limit format pattern: "number/HH:MM:SS"
        pattern = r"^\d+/\d{2}:\d{2}:\d{2}$"
        if not re.match(pattern, v):
            raise ValueError(
                f"Invalid rate limit format '{v}'. Expected format: 'count/HH:MM:SS' (e.g., '100/00:00:01')"
            )

        # Extract count and validate it's at least 1
        count = int(v.split("/")[0])
        if count < 1:
            raise ValueError(
                f"Rate limit count must be at least 1, got {count}. "
                "A zero rate limit would disable order submission entirely."
            )
    ...
    def from_env(cls) -> "TradingNodeSettings":
        ...
        binance = None
        if "BINANCE_API_KEY" in os.environ and os.environ["BINANCE_API_KEY"]:
            if "BINANCE_API_SECRET" not in os.environ or not os.environ["BINANCE_API_SECRET"]:
                raise ValueError(
        ...
        bybit = None
        if "BYBIT_API_KEY" in os.environ and os.environ["BYBIT_API_KEY"]:
            if "BYBIT_API_SECRET" not in os.environ or not os.environ["BYBIT_API_SECRET"]:
                raise ValueError(
                    "BYBIT_API_KEY is set but BYBIT_API_SECRET is missing. "
                    "Both must be provided together."
                )
