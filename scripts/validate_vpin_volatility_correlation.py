        ...
        if len(analysis_df) > lag + 10:
            ...
            if valid_mask.sum() > 10:
                lag_corr, _ = stats.pearsonr(
                    analysis_df.loc[valid_mask, "vpin"], lagged_vol[valid_mask]
                )
...
def main():
    ...
    for period_name, (start_dt, end_dt) in flash_crash_periods.items():
        ...
        try:
            ...
            max_drawdown = (
                (min(prices) - max(prices[: len(prices) // 2]))
                / max(prices[: len(prices) // 2])
                * 100
            )
            ...
            print(
                f"    Spearman r:  {results['spearman_correlation']:.4f} (p={results['spearman_pvalue']:.2e})"
            )
            print(
                f"    VPIN: mean={results['vpin_mean']:.4f}, std={results['vpin_std']:.4f}, max={results['vpin_max']:.4f}"
            )
            print(
                f"    Volatility: mean={results['volatility_mean']:.6f}, max={results['volatility_max']:.6f}"
            )
    ...
    if len(all_results) > 0:
        ...
        print(
            f"    Low VPIN (<0.4): {len(low_vpin)} observations ({100 * len(low_vpin) / len(all_merged):.1f}%)"
        )
        ...
        for lag in [1, 2, 3, 6, 12]:
            if len(all_merged) > lag + 10:
                ...
                if valid_mask.sum() > 10:
                    lag_corr, _ = stats.pearsonr(
                        all_merged.loc[valid_mask, "vpin"], lagged_vol[valid_mask]
                    )
