...
class PriceTargetCalculator:
    ...
    def atr_probability_targets(
        ...
        """
        probabilities = {1.0: 0.6827, 2.0: 0.9545, 3.0: 0.9973}
    ...
    def range_based_targets(
        ...
        targets = {"adr": adr, "method": "Range Based"}
    ...
    def composite_analysis(
        ...
        if atr:
            ...
            all_targets.update(
                {f"atr_{k}": v for k, v in atr_t.items() if isinstance(v, (int, float))}
            )

        # 2. Measured Move
        mm_t = self.measured_move(swing_low, swing_high, retracement_end)
        all_targets.update({f"mm_{k}": v for k, v in mm_t.items() if isinstance(v, (int, float))})

        # 3. Fibonacci
        fib_t = self.fibonacci_extensions(swing_low, swing_high, retracement_end, direction)
        all_targets.update({k: v for k, v in fib_t.items() if k.startswith("fib_")})

        # 4. StdDev ICT
        std_t = self.std_channel_targets(swing_high, swing_low, direction)
        all_targets.update({f"std_{k}": v for k, v in std_t.items() if isinstance(v, (int, float))})
...
def pnf_count_simple(
    num_columns: int, box_size: float, reversal: int, base_price: float, direction: str = "up"
) -> float:
    """
...
if __name__ == "__main__":
    ...
    targets = calculate_target_quick(
        entry=100.0, swing_high=115.0, swing_low=90.0, atr=4.5, direction="up"
    )
    ...
    fib = calc.fibonacci_extensions(
        swing_low=100, swing_high=150, retracement_end=130, direction="up"
    )
    ...
    for name, value in sorted(
        fib.items(), key=lambda x: x[1] if isinstance(x[1], (int, float)) else 0
    ):
